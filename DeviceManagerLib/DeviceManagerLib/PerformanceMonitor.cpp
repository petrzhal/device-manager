...
namespace dm {
    ...
    PerformanceMonitor::DisksStorage PerformanceMonitor::getDiskUsage() const {
        ...
        static const auto existedDisks = std::views::iota(0, ('Z' - 'A') + 1) | std::views::filter([&](int i) -> bool { return driveMask & (1 << i); }) | std::views::transform([](int diskN) -> std::string { return std::format("{}:\\", static_cast<char>('A' + diskN)); }) | std::views::filter([](std::string disk) -> bool { return GetDriveTypeA(disk.data()) == DRIVE_FIXED; }) | std::ranges::to<std::vector>();
